...
class Line:
    ...
    def _build_line_data(self, station_df):
        ...
        line = [
            Station(station_data["station_id"].unique()[0],
                    stations[0],
                    self.color)
        ]
        prev_station = line[0]
    ...
    def run(self, timestamp, time_step):
        """
        Advances trains between stations in the simulation. Runs
        turnstiles.
        """
    ...
    def _advance_turnstiles(self, timestamp, time_step):
        ...
        _ = [station.turnstile.run(timestamp, time_step) for station in
             self.stations]
    ...
    def _next_train(self, start_index=0, b_direction=True, step_size=1):
        ...
        if b_direction is True:
            ...
            if curr_index == -1:
                curr_index = self._next_train_a(len(self.stations) - 1,
                                                step_size)
                b_direction = False
        else:
    ...
    def _get_next_idx(self, curr_index, b_direction, step_size=None):
        """Calculates the next station index. Returns next index and if it is
        b_direction"""
